...
of CORDEX)
- cordex_ba: 'ba' for baseline. A numpy array with the climatological data.
    Its dimensions are
    [len(qus), len(oases), len(years), len(months), len(models)].
    Let's say you want the dew point temperature from 'NOAA-GFDL-GFDL-ESM2M'
    for Skoura in Nov 1992. It would be cordex_ba[5, 3, 2, 10, 8].
    5 is the index number of dew point, 3 is the index number of Skoura,
    2 is the index number of 1992, 10 is the index number of Nov, 8 is the
    index number of the NOAA model.
- cordex_85: The same as cordex_ba, but the years are now the 30 years
    starting from 2071 for the rcp85 scenario
- cordex_26 : Same as cordex_85, but for rcp2.6. It has a lot of np.nan
    because we only have 3 cordex models running the rcp2.6 scenario
'''
...
with open(path_data, 'rb') as f:
    qus, oases, years, months, models, cordex_ba, cordex_85, cordex_26 = \
        pickle.load(f)
...
for o in oases:
    ...
    # temperature: RCP 2.6
    diff = remove_ba(cordex_26[idx_qus, idx_oases, ...],
                     cordex_ba[idx_qus, idx_oases, ...])
    t_data = np.append(t_data, diff)
    ndata = len(diff)

    # temperature: RCP 8.5
    diff = remove_ba(cordex_85[idx_qus, idx_oases, ...],
                     cordex_ba[idx_qus, idx_oases, ...])
    t_data = np.append(t_data, diff)

    # temperature: GEM
    diff = remove_ba(gem_85[idx_qus, idx_oases, ...],
                     gem_ba[idx_qus, idx_oases, ...])
...
data = pd.DataFrame(data={"oases": o_data, "Model": m_data,
                          "temperature": t_data, "precipitation": p_data})
...
fig.add_subplot(211)
ax = sns.violinplot(x="oases", y="temperature", data=data, hue="Model",
...
# add GEM models
plt.scatter(np.arange(len(t_gem_data))+.1, t_gem_data, marker="*",
            color="w", alpha=1., s=50)
...
ax = sns.violinplot(x="oases", y="precipitation", data=data_sub, hue="Model",
                    split=True, inner="quartile")
...
# add GEM MODELS
plt.scatter(np.arange(len(p_gem_data))+.1, p_gem_data, marker="*", color="w",
            alpha=1., s=50)
