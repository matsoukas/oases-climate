...
@author: tgrassi, Roberto Ingrosso, trivial modifications by Christos Matsoukas
"""
...
years_ba is a list with the names of the years in the baseline.
     CAUTION: So far we have been using 30 years 1990-2019.
     Now it's 70 years 1951-2020. The end year is different!
...
cordex_85: The same as cordex_ba, but the years are now the 80 years starting
    from 2021 for the rcp85 scenario
cordex_26 : Same as cordex_85, but for rcp2.6. It has a lot of np.nan because
    we only have 3 cordex models running the rcp2.6 scenario
'''

path = "../data/data_pr_evap.pkl"
path_save = "FigS2.png"

with open(path, 'rb') as f:
    qus, oases, years_ba, years_rcp, months, models, cordex_ba, cordex_85, \
        cordex_26 = pickle.load(f)
...
# concatenate baseline data and models data to have 1951-2100,
# [qus, oases, years, models]
...
for ioasis, o in enumerate(oases):
    ...
    hist, bins = np.histogram(yy, bins=nbins, range=[min(data_years),
                                                     max(data_years)])
    bins_center = (bins[1:] + bins[:-1]) / 2.

    # MK test wet
    mk_result = mk.original_test(hist[bins_center >= 2020])
    print("dry %s %s %+f    %e" % (o.ljust(20), mk_result.trend.ljust(10),
    ...
    # MK test wet
    mk_result = mk.original_test(hist[bins_center >= 2020])
    print("wet %s %s %+f    %e" % (o.ljust(20), mk_result.trend.ljust(10),
                                   mk_result.slope, mk_result.p))

    # plot corresponding bars (wet)
    ax.bar(bins_center + 2, hist, width=4, color="xkcd:bright sky blue",
           label="wet")
