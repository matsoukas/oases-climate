...
def bootstrap(cordex, what, rcp_label, axs, idx, jdx, nboot=int(1e5)):
    ...
    print("bootstrapping %s temperatures with %d models..." % (rcp_label,
                                                               nboot))
    ...
    for j, o in enumerate(tqdm(oases)):
        ...
        # slice the data for the given quantity and oases, obtain
        # data_*[years, models]
        ...
        for i in range(nboot):
            ...
            diff_boot[j, i] = np.mean(data_cdx_flat_boot) - \
                np.mean(data_ba_flat_boot)
    ...
    for i in range(1, 4):
        ...
        for j in range(noases):
            hx = Rectangle((j - 0.3, mean[j] - i*std[j]), 0.6, 2*i*std[j],
                           alpha=0.2)
            ax.add_patch(hx)
    ...
    ax.scatter(np.arange(noases), diff, color="tab:orange", zorder=999,
               marker="o")
