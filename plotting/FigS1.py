...
yoff = 4  # offset year for decades, year_ref+yoff, eg 1983+4 starts from 1987
...
ylabel = "Temperature change \n from 1983-2012 avg (°C)"  # label on y axis
...
# store data per decade as average of the differences with the baseline,
# [decades, oases]
...
for i, o in enumerate(oases):
    ...
    print("%s %s %+f    %e" % (oases[i].ljust(30), mk_result.trend.ljust(30),
...
for j, y in enumerate(ydata):
    ...
    # plot the bar
    plt.bar(xdata+j*wbar-ndecs*wbar/2+wbar/2, y, color=cmap(cnorm),
            width=wbar, zorder=99, alpha=0.7, label=label)
...
cmap = matplotlib.colors.ListedColormap(['xkcd:brown', 'xkcd:clay brown',
                                         'xkcd:greenblue',
